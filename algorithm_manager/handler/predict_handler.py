...
def sequence_validation(y_true,y_predict,tlabel):
    ...
    # y_predict = np.around(y_predict,places)
    ...
    bdict['predictValue'] = [round(i,places) for i in y_predict.flatten().tolist()]
    bdict['均方误差'] = mse
    bdict['平均绝对误差'] = mae
    return bdict
...
def regression_validation(y_test,y_predict,tlabel):
    ...
    # y_predict1 = np.around(y_predict,places)
    # print(y_predict1)
...
def model_validate():
    ...
    if algorithm in tools.reg_list:
        ...
        for i in range(len(data_all) - sequence_length):
            ...
            label.append(test[test.columns[-1]].values[i + sequence_length])
            timeseq.append(test[test.columns[0]].values[i + sequence_length])
...
def model_predict():
    ...
    if algorithm in tools.reg_list:
        ...
        scalery_path = os.path.join(tools.ModelPath, model_id + '.label')
        scaler_y = joblib.load(scalery_path)
        y_predict = sequence_predict(test,model1,model,scaler_y)
        bdict['predictValue'] = [round(i,4) for i in y_predict.flatten().tolist()]
